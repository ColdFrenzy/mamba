...
class Switch(WorldObj):
    def __init__(self, world, index):
        super().__init__(world, 'switch', world.IDX_TO_COLOR[index])
...
class GroundSwitch(WorldObj):
    # differently from switch, ground_switch don't need to be actively toggled,
    # it will be toggled when the agent is on it
    def __init__(self, world, untoggled_index, toggled_index):
        self.untoggled_index = untoggled_index
        self.toggled_index = toggled_index
        self.toggled_color = world.IDX_TO_COLOR[toggled_index]
        self.untoggled_color = world.IDX_TO_COLOR[untoggled_index]
        self.toggled = False
        # create an python inline if statement to check if there is an agent on the ground_switch
        super().__init__(world, 'ground_switch', world.IDX_TO_COLOR[self.untoggled_index])
    def can_overlap(self):
        return True

    def encode(self, world, current_agent=False):
        """Encode the a description of this object as a 3-tuple of integers. The o"""
        # State, 0: untoggled, 1: toggled
        if self.toggled:
            state = 1
        else:
            state = 0
    ...
    def render(self, img):
        if self.toggled: # if it's toggled, render it as the toggled color
            fill_coords(img, point_in_rect(0, 1, 0, 1), COLORS[self.toggled_color])
        elif self.toggled == False:
            fill_coords(img, point_in_rect(0, 1, 0, 1), COLORS[self.untoggled_color])
...
class MultiGridEnv(gym.Env):
    ...
    def __init__(
        ...
        self.agents = agents
        # Keep track of the previous cell each agent was in
        self.prev_agents_pos = [None for _ in range(len(self.agents))]
        self.num_toggled_switches = 0
        ...
        if partial_obs:
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(agent_view_size, agent_view_size, self.objects.encode_dim if not rgb_obs else 3),
                dtype='uint8'
            )
            ...
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(width, height, self.objects.encode_dim if not rgb_obs else 3),
                dtype='uint8'
            )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.partial_obs:
            ...
            obs = [self.grid.encode(self.objects) for _ in range(len(self.agents))]
    ...
    def _handle_ground_switch(self,i , rewards, fwd_pos, fwd_cell):
        fwd_cell.toggled = True
    ...
    def step(self, actions):
        ...
        # randomly pick agent order
        # order = np.random.permutation(len(actions))
        # deterministically pick agent order
            ...
            # Move forward
                ...
                if fwd_cell is not None:
                    if fwd_cell.type == 'goal':
                        ...
                        self.prev_agents_pos[i] = None

                    elif fwd_cell.type == 'switch':
                        self._handle_switch(i, rewards, fwd_pos, fwd_cell)
                        self.prev_agents_pos[i] = None

                    elif fwd_cell.type == 'ground_switch':
                        # we toggle the ground switch
                        if fwd_cell.toggled == False:
                            self.num_toggled_switches += 1
                            if self.num_toggled_switches == self.num_switches:
                                for door_pos in self.door_pos:
                                    self.grid.get(*door_pos).open()
                        self._handle_ground_switch(i, rewards, fwd_pos, fwd_cell)
                        self.grid.set(*fwd_pos, self.agents[i])
                        self.grid.set(*self.agents[i].pos, self.prev_agents_pos[i])
                        self.agents[i].pos = fwd_pos
                        self.prev_agents_pos[i] = fwd_cell
        ...
        if self.partial_obs:
            ...
            obs = [self.grid.encode(self.objects) for _ in range(len(actions))]
    ...
    def get_obs(self):
        return [np.moveaxis(obs,-1,0) for obs in self.gen_obs()]
    def get_state(self):
        return np.concatenate(self.get_obs(), axis=0).astype(np.float32)
    
    def get_avail_actions(self):
        test = [np.ones(self.ac_dim) for _ in range(len(self.agents))]
        return test
    def get_state_size(self):
        """Returns the size of the global state."""
        obs_shape = self.gen_obs()[0].shape
        return (obs_shape[2] * len(self.agents), obs_shape[0], obs_shape[1])
